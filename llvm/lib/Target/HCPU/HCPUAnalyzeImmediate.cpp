//===-- HCPUAnalyzeImmediate.cpp - Analyze Immediates ---------------------===//
//
...
//
//===----------------------------------------------------------------------===//
#include "HCPUAnalyzeImmediate.h"
#include "HCPU.h"
...
HCPUAnalyzeImmediate::Inst::Inst(unsigned O, unsigned I) : Opc(O), ImmOpnd(I) {}

// Add I to the instruction sequences.
void HCPUAnalyzeImmediate::AddInstr(InstSeqLs &SeqLs, const Inst &I) {
...
void HCPUAnalyzeImmediate::GetInstSeqLsADDiu(uint64_t Imm, unsigned RemSize,
                                             InstSeqLs &SeqLs) {
  GetInstSeqLs((Imm + 0x8000ULL) & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ADDiu, Imm & 0xffffULL));
}

void HCPUAnalyzeImmediate::GetInstSeqLsORi(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
  GetInstSeqLs(Imm & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ORi, Imm & 0xffffULL));
}

void HCPUAnalyzeImmediate::GetInstSeqLsSHL(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
...
void HCPUAnalyzeImmediate::GetInstSeqLs(uint64_t Imm, unsigned RemSize,
                                        InstSeqLs &SeqLs) {
...
// are replaced with
//  LUi 0x444
void HCPUAnalyzeImmediate::ReplaceADDiuSHLWithLUi(InstSeq &Seq) {
...
void HCPUAnalyzeImmediate::GetShortestSeq(InstSeqLs &SeqLs, InstSeq &Insts) {
...
const HCPUAnalyzeImmediate::InstSeq
&HCPUAnalyzeImmediate::Analyze(uint64_t Imm, unsigned Size,
