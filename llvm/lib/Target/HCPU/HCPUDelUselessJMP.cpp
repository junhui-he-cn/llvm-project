//===-- HCPUDelUselessJMP.cpp - HCPU DelJmp -------------------------------===//
//
...
runOnMachineBasicBlock(MachineBasicBlock &MBB, MachineBasicBlock &MBBN) {
  ...
  if (I->getOpcode() == HCPU::JMP && I->getOperand(0).getMBB() == &MBBN) {
...
/// createHCPUDelJmpPass - Returns a pass that DelJmp in HCPU MachineFunctions
FunctionPass *llvm::createHCPUDelJmpPass(HCPUTargetMachine &tm) {
