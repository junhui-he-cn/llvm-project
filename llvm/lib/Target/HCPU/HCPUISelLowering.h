...
protected:
  ...
  SDValue getAddrLocal(NodeTy *N, EVT Ty, SelectionDAG &DAG) const {
    SDLoc DL(N);
    unsigned GOTFlag = HCPUII::MO_GOT;
    SDValue GOT = DAG.getNode(HCPUISD::Wrapper, DL, Ty, getGlobalReg(DAG, Ty),
                              getTargetNode(N, Ty, DAG, GOTFlag));
    SDValue Load =
        DAG.getLoad(Ty, DL, DAG.getEntryNode(), GOT,
                    MachinePointerInfo::getGOT(DAG.getMachineFunction()));
    unsigned LoFlag = HCPUII::MO_ABS_LO;
    SDValue Lo =
        DAG.getNode(HCPUISD::Lo, DL, Ty, getTargetNode(N, Ty, DAG, LoFlag));
  ...
  SDValue getAddrGlobal(NodeTy *N, EVT Ty, SelectionDAG &DAG, unsigned Flag,
    ...
    SDLoc DL(N);
    SDValue Tgt = DAG.getNode(HCPUISD::Wrapper, DL, Ty, getGlobalReg(DAG, Ty),
  ...
  SDValue getAddrGlobalLargeGOT(NodeTy *N, EVT Ty, SelectionDAG &DAG,
    ...
    SDLoc DL(N);
    SDValue Hi =
        DAG.getNode(HCPUISD::Hi, DL, Ty, getTargetNode(N, Ty, DAG, HiFlag));
    Hi = DAG.getNode(ISD::ADD, DL, Ty, Hi, getGlobalReg(DAG, Ty));
    SDValue Wrapper = DAG.getNode(HCPUISD::Wrapper, DL, Ty, Hi,
  ...
  SDValue getAddrNonPIC(NodeTy *N, EVT Ty, SelectionDAG &DAG) const {
    SDLoc DL(N);
    SDValue Hi = getTargetNode(N, Ty, DAG, HCPUII::MO_ABS_HI);
    SDValue Lo = getTargetNode(N, Ty, DAG, HCPUII::MO_ABS_LO);
    return DAG.getNode(ISD::ADD, DL, Ty, DAG.getNode(HCPUISD::Hi, DL, Ty, Hi),
...
private:
  ...
  isEligibleForTailCallOptimization(const HCPUCC &HCPUCCInfo,
                                    ...
                                    const HCPUFunctionInfo &FI) const = 0;
